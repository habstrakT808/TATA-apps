...
service cloud.firestore {
  ...
  Future<String> _getOrCreateChatFirestore(String userId, String orderId) async {
    try {
      // Log untuk debugging
      print('Creating chat for user $userId and order $orderId');
      ...
      if (existingChatQuery.docs.isNotEmpty) {
        print('Found existing chat for order $orderId: ${existingChatQuery.docs.first.id}');
        return existingChatQuery.docs.first.id;
      } else {
        print('No existing chat found for order $orderId. Creating new chat...');
      }
      
      // Jika belum ada, cari admin yang tersedia dan buat chat baru
      final adminId = await findAvailableAdmin();
      print('Selected admin ID for new chat: $adminId');
      ...
      // Deteksi kategori produk dari order ID
      final kategori = detectCategoryFromReference(orderId);
      
      // Buat chat baru dengan referensi pesanan yang ditentukan
      final docRef = await _firestore.collection('chats').add({
        'user_id': userId,
        'admin_id': adminId,
        'order_reference': orderId,
        'kategori': kategori,  // Tambahkan kategori saat membuat chat
      ...
      print('Created new chat with ID: $chatId for order $orderId with kategori: $kategori');
...
service cloud.firestore {
  ...
  Future<String> _getOrCreateChatLaravelFallback(String userId, String orderId) async {
    try {
      print('Getting or creating chat via API fallback for user $userId and order $orderId');
      ...
      final response = await http.post(
        Uri.parse('http://localhost:8000/api/mobile/chat/get-or-create'),
        headers: {
      ...
      if (response.statusCode == 200 && responseData['status'] == 'success') {
        ...
        print('Chat ID from API: $chatId for order $orderId');
        return chatId;
      } else {
        throw Exception('API error: ${responseData['message'] ?? 'Gagal membuat chat'}');
      }
    } catch (e) {
  ...
  Future<void> _sendMessageLaravelFallback(String chatId, String content, String senderType) async {
    ...
    // Kirim pesan via API
    final response = await http.post(
      Uri.parse('http://localhost:8000/api/mobile/chat/send'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
        'Accept': 'application/json',
  ...
  // Helper method to detect category from order reference or any string
  String detectCategoryFromReference(String reference) {
    if (reference == null || reference.isEmpty) {
      return 'Logo'; // Default if empty
    }
    
    // Normalize the reference string for consistent detection
    final normalizedRef = reference.trim().toLowerCase();
    
    // More comprehensive detection patterns
    if (normalizedRef.contains('poster') || 
        normalizedRef.contains('flyer') || 
        normalizedRef.contains('brosur')) {
      return 'Poster';
    } else if (normalizedRef.contains('banner') || 
               normalizedRef.contains('spanduk') || 
               normalizedRef.contains('umbul')) {
      return 'Banner';
    } else if (normalizedRef.contains('logo') || 
               normalizedRef.contains('brand') || 
               normalizedRef.contains('icon')) {
      return 'Logo';
    }
    
    // Check for UUID pattern and use specific rules
    if (normalizedRef.contains('-') && normalizedRef.length > 30) {
      // For UUIDs, use a more balanced approach based on multiple characters
      // to avoid biasing toward any specific category
      final cleanUuid = normalizedRef.replaceAll('-', '');
      int sum = 0;
      
      // Sum the character codes of the first 4 characters
      for (int i = 0; i < 4 && i < cleanUuid.length; i++) {
        sum += cleanUuid.codeUnitAt(i);
      }
      
      // Distribute more evenly between categories
      if (sum % 3 == 0) {
        return 'Logo';
      } else if (sum % 3 == 1) {
        return 'Banner';
    ...
    // Default to Logo if nothing detected
    return 'Logo';
  }
  
  // Helper method untuk default data pesanan
  Map<String, dynamic> _getDefaultPesananData({String? kategori}) {
    // Kategori default berdasarkan parameter atau default ke 'Logo'
    String useKategori = kategori ?? 'Logo';
    
    // Normalisasi kategori
    useKategori = useKategori.trim();
    if (useKategori.isEmpty) {
      useKategori = 'Logo';
    }
    
    // Log untuk debugging
    print('Creating default pesanan data with kategori: $useKategori');
    
    // Set harga default berdasarkan kategori
    int defaultHarga = 100000; // Default untuk Logo
    
    if (useKategori.toLowerCase().contains('poster')) {
      defaultHarga = 300000;
      useKategori = 'Poster';
    } else if (useKategori.toLowerCase().contains('banner')) {
      defaultHarga = 200000;
      useKategori = 'Banner';
    } else {
      // Default to Logo for anything else
      useKategori = 'Logo';
    }
    
    return {
      'judul': 'Desain ${capitalize(useKategori)}',
      'kategori': useKategori,
      'kelas_jasa': 'Standard',
      'harga_paket_jasa': defaultHarga,
      'deskripsi': '',
      'gambar_referensi': '',
      ...
      if (token != null) {
        try {
          ...
          final response = await http.get(
            Uri.parse('http://localhost:8000/api/mobile/chat/messages/$cleanOrderReference'),
            headers: {
              'Authorization': 'Bearer $token',
              'Accept': 'application/json',
            },
  ...
  Future<Map<String, dynamic>> getPesananData(String orderReference) async {
    try {
      ...
      // Variabel untuk menyimpan kategori jika ditemukan
      String detectedCategory = detectCategoryFromReference(cleanOrderReference);
      print("Initial category detection: $detectedCategory");
      
      // Default response jika tidak ada data
      final defaultData = _getDefaultPesananData(kategori: detectedCategory);

      // Coba dapatkan data dari Firestore terlebih dahulu
      try {
        // Cek di koleksi chats untuk chat yang memiliki order_reference ini
        ...
        if (chatQuery.docs.isNotEmpty) {
          // Periksa semua dokumen chat untuk mencari kategori
          for (var doc in chatQuery.docs) {
            final chatData = doc.data();
            
            // Coba ekstrak kategori dari chatData jika ada
            if (chatData.containsKey('kategori') && 
                chatData['kategori'] != null && 
                chatData['kategori'].toString().isNotEmpty) {
              detectedCategory = chatData['kategori'].toString();
              print('Kategori ditemukan di chat: $detectedCategory');
              break; // Keluar dari loop jika kategori ditemukan
            }
          }
          
          // Ambil data admin jika ada
          final firstChatData = chatQuery.docs.first.data();
          final adminId = firstChatData['admin_id'];
      ...
      try {
        ...
        if (token != null) {
          ...
          List<String> possibleEndpoints = [
            'http://localhost:8000/api/mobile/pesanan/$cleanOrderReference',
            'http://localhost:8000/api/pesanan/$cleanOrderReference',
            'http://localhost:8000/api/mobile/orders/$cleanOrderReference',
            'http://localhost:8000/api/orders/$cleanOrderReference',
            'http://localhost:8000/api/mobile/order/$cleanOrderReference',
            'http://localhost:8000/api/order/$cleanOrderReference',
          ];
          ...
          for (String endpoint in possibleEndpoints) {
            try {
              ...
              ).timeout(Duration(seconds: 3)); // Shorter timeout to avoid hanging
              
              print('API Response Code for $endpoint: ${response.statusCode}');
              ...
              if (response.statusCode == 200) {
                ...
                if (data['status'] == 'success' && data['data'] != null) {
                  ...
                  // Ekstrak kategori dari API jika ada
                  if (pesananData['kategori'] != null && pesananData['kategori'].toString().isNotEmpty) {
                    detectedCategory = pesananData['kategori'].toString();
                    print('Kategori dari API: $detectedCategory');
                    
                    // Store this category in Firestore for future reference
                    try {
                      await _firestore.collection('order_categories').doc(cleanOrderReference).set({
                        'kategori': detectedCategory,
                        'updated_at': FieldValue.serverTimestamp()
                      }, SetOptions(merge: true));
                    } catch (e) {
                      print('Error storing category in Firestore: $e');
                    }
                  }
                  
                  // Log details untuk debugging
                  final kategori = detectedCategory;
                  print('Judul: Desain ${capitalize(kategori)}');
                  print('Kategori: $kategori');
                  print('Kelas: ${pesananData['kelas_jasa'] ?? 'Standard'}');
                  
                  int harga = 100000;  // Default untuk Logo
                  ...
                  int hargaPaketJasa = int.tryParse(pesananData['harga_paket_jasa']?.toString() ?? '') ?? harga;
                  print('Harga: $hargaPaketJasa');
                  print('Gambar: ${pesananData['gambar_referensi'] ?? ''}');
                  
                  apiSuccess = true;
                  
                  return {
                    'judul': 'Desain ${capitalize(kategori)}',
                    'kategori': kategori,
                    'kelas_jasa': pesananData['kelas_jasa'] ?? 'Standard', 
                    'harga_paket_jasa': hargaPaketJasa,
      ...
      if (!apiSuccess) {
        print('All API endpoints failed, using detected category from reference');
        
        // Check if we have stored category information in Firestore
        try {
          final categoryDoc = await _firestore.collection('order_categories').doc(cleanOrderReference).get();
          if (categoryDoc.exists) {
            final categoryData = categoryDoc.data() as Map<String, dynamic>;
            if (categoryData['kategori'] != null && categoryData['kategori'].toString().isNotEmpty) {
              detectedCategory = categoryData['kategori'].toString();
              print('Using stored category from Firestore: $detectedCategory');
            }
          }
        } catch (e) {
          print('Error fetching stored category: $e');
        }
      }
      
      // Create optimized response based on detected category
      final optimizedResponse = _getDefaultPesananData(kategori: detectedCategory);
      optimizedResponse['admin_name'] = defaultData['admin_name'];
      
      print('Data pesanan yang didapat: $optimizedResponse');
      return optimizedResponse;
    } catch (e) {
      print('Error in getPesananData: $e');
      
      // Even if everything fails, return a valid default response
      final fallbackCategory = 'Logo';
      final fallbackResponse = _getDefaultPesananData(kategori: fallbackCategory);
      print('Using ultimate fallback data with kategori: $fallbackCategory');
