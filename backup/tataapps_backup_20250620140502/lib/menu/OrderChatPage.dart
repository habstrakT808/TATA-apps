  ...
  Future<void> _loadChat() async {
    ...
    try {
      ...
      if (unreadMessages.isNotEmpty) {
        // Get the chat UUID first, then mark messages as read
        final chats = await ChatApiService.getChatRooms();
        final chatForOrder = chats.firstWhere(
          (chat) => chat.pesananId == widget.pesananUuid,
          orElse: () => ChatRoom(
            id: '', 
            pesananId: '', 
            product: ProductChat(
        ...
        if (chatForOrder.id.isNotEmpty) {
          await ChatApiService.markMessagesAsRead(chatForOrder.id);
        }
      }
    } catch (e) {
