...
class _RegisterState extends State<Register> {
  ...
  Future<void> _CekEmail() async {
    try {
      CustomWidget.NotifLoading(context); // Tambahkan loading dialog
      ...
      if (result != null) {
        if (result['status'] == "success") {
          ...
          CustomWidget.NotifGagalText(
              context, result['message'], 14);
        }
      } else {
        // Terjadi error komunikasi dengan server
        print("gagal : null");
        // Jika API cek email gagal, maka coba langsung lanjut ke halaman berikutnya
        // untuk menghindari masalah 404 Not Found
        final registerDATA = RegisterData(
          namaUser: _namaController.text,
          email: _emailController.text,
          noHp: _noHpController.text,
      ...
      CustomWidget.NotifGagalText(
          context, "Terjadi kesalahan, silakan coba lagi", 14);
    }
  }
